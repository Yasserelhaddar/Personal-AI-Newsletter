...
from src.infrastructure.error_handling import handle_agent_errors
...
async def collect_content_parallel(state: NewsletterGenerationState) -> NewsletterGenerationState:
    ...
    try:
        ...
        logger.warning(f"Failed to initialize Firecrawl API client: {e}")
        add_error(
            state,
            ProcessingStage.COLLECTION,
            f"Failed to initialize Firecrawl API client: {str(e)}",
            ErrorSeverity.MEDIUM,
            "FIRECRAWL_INIT_FAILED",
        )
    ...
    try:
        ...
        add_error(
            state,
            ProcessingStage.COLLECTION,
            f"Failed to connect to GitHub MCP server: {str(e)}",
            ErrorSeverity.MEDIUM,
